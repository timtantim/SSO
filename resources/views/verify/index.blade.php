       ...
       <table class="table table-bordered" id="table">
          ...
          @foreach($users as $key => $value)
             <tr>
                ...
                <td>{{($value->verify=='0')?'尚未驗證':'已驗證'}}</td>
                <td>{{$value->created_at}}</td>
                <td>{{$value->updated_at}}</td>
                <td>

                     <a href="#" class="edit-modal btn btn-warning btn-sm" data-id="{{$value->id}}" data-name="{{$value->name}}" data-verify="{{$value->verify}}">
                      <i class="fa fa-pencil" aria-hidden="true"></i>
                     </a>

                     <a href="#" class="delete-modal btn btn-danger btn-sm" data-id="{{$value->id}}" data-name="{{$value->name}}" data-verify="{{$value->verify}}">
      ...
      <div class="modal-content">
          ...
          <div class="modal-body">
              <form class="form-horizontal" role="modal">
                  ...
                  <div class="form-group">
                      <label class="control-label col-sm-2" for="english">驗證狀態</label>
                      <div class="col-sm-10">
  ...
  $(document).ready(function() {
    $('.edit-modal').bind('click',function(){
      console.log($(this).data('chinese'));
            ...
            $('#user_verify').val($(this).data('verify'));
            $('#myModal').modal('show');
    });
    ...
    $('#verify_user_button').bind('click',function(){
      ...
      if(get_verify_select=='1'){
        ...
        $.ajax({
                       type: 'POST',
                       url: 'http://'+window.location.host+'/api/verifyuser',
                       async:false,
                       data: user_data,   
